...
**Junior**: "What are the main SRE org models?" — Embedded (an SRE sits inside each product team), central (one platform SRE team serves many teams via self-service), and hybrid (a central team for platform and standards plus embedded SREs for product-specific depth).

**Mid**: "What are the tradeoffs between embedded and central SRE?" — Embedded gives deep product context and tight integration but disperses SREs, weakens their community, and risks drifting into pure ops; central builds a strong SRE community and a reusable platform with consistent standards but sits further from the product and can become a bottleneck.

**Senior**: "How do you pick the right model for an org?" — Match it to size and maturity: start central while doing everything, embed for mature or specialized critical products, and move to hybrid as you scale; preserve SRE identity by giving embedded SREs a solid reporting line into SRE leadership (dotted line to product) so they don't become ops-only.

**Staff**: "How do you design an SRE org that stays healthy?" — Invest in a self-service platform so the central team scales instead of bottlenecking, staff roughly 1 SRE per 10–20 devs, enforce the >50% engineering / toil-cap split with compensation parity and real career paths, and actively prevent the anti-patterns where SREs become on-call slaves or janitors with no engineering time.
