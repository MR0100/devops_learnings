...
**Junior**: "What is a runbook?" — A step-by-step guide tied to a specific alert or scenario that lists symptoms, likely causes, concrete investigation commands, and mitigations so on-call can resolve it quickly and reduce MTTR.

**Mid**: "What makes a runbook actually useful at 3 a.m.?" — Specific copy-pasteable commands instead of vague prose like "check the database," likely-causes and mitigations ordered cheapest-first, a clear escalation trigger, and a link in the alert annotation so the on-call reaches it in one click.

**Senior**: "How do you keep runbooks from going stale?" — Treat them as living documents updated as a standing postmortem action item, version-control them in Markdown, and run quarterly drills where an engineer follows the runbook cold to surface broken commands and gaps before a real incident does.

**Staff**: "How do you use runbooks to enable on-call across an org?" — Enforce a no-runbook-no-page policy so every alert is documented and actionable, standardize a template (alert, symptoms, causes, investigation, mitigation, escalation), onboard new on-call by drilling the top runbooks, and push the easy steps toward auto-remediation so the runbook becomes code rather than a manual ritual.
