...
**Junior**: "What is SRE in one sentence?" — SRE is Google's discipline of treating operations as a software problem: engineers apply software engineering to reliability, with the goal of a reliable, scalable service rather than manual firefighting.

**Mid**: "How does SRE differ from DevOps?" — DevOps is a broad culture of breaking down dev/ops silos, while SRE is a concrete, opinionated implementation of those principles with specific practices like SLOs, error budgets, blameless postmortems, and a ~50% toil cap.

**Senior**: "How do SLOs and error budgets actually change engineering behavior?" — An SLO sets the reliability target and its complement is the error budget; when budget remains, teams ship features and take risks, and when it's exhausted the error-budget policy freezes risky changes and redirects effort to reliability, making the speed/reliability tradeoff data-driven instead of political.

**Staff**: "How would you structure SRE in a growing org?" — Start embedded so SREs build product context, evolve toward a central platform team that owns shared tooling and self-service, and gate engagements on SLOs and a PRR so SRE capacity goes to services that meet a reliability bar rather than absorbing unbounded toil.
