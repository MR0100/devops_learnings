...
**Junior**: "What is headroom?" — Headroom is the gap between current load and capacity as a fraction — (capacity − load) / capacity — so 700 load on 1000 capacity is 30% headroom; the typical target is 30–50% with about 20% as a floor.

**Mid**: "Why keep headroom, and how do you track it?" — You need it to absorb spikes, survive a zone failover, cover maintenance, and bridge growth between scaling events; track it per resource (CPU, memory, DB connections, IO) and alert when any dimension drops below ~20% so you get weeks of notice before saturation.

**Senior**: "How does headroom interact with multi-zone failover and autoscaling?" — If losing a zone shifts its load to survivors, each surviving zone must carry that extra — three zones at 33% need ~50% headroom so two can absorb the load when one fails — and even with autoscaling you must set max replicas and alert before approaching them, because hitting the cap is a latency cliff.

**Staff**: "How do you manage headroom at scale across cost and many teams?" — Tier headroom targets by criticality (critical ~50%, important ~30%, internal ~20%) since more headroom means more cost, give teams resource budgets with usage tracking, re-test periodically because fixing one bottleneck shifts it elsewhere, and tie a quarterly capacity roadmap to pre-buy commits and failover reserves.
