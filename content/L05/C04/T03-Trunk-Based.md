...
- Commit to `main` (or merge tiny PRs) at least daily; the smaller and more frequent the integration, the cheaper the conflicts.
- Gate every incomplete change behind a feature flag so `main` is always releasable even with work in progress.
- Invest in fast, reliable CI and a comprehensive automated test suite first — trunk-based amplifies whatever your test discipline already is.
- Keep PRs reviewable in minutes (rough rule: under ~200-400 lines); use stacked PRs to split larger efforts.
- Rebase onto `origin/main` before pushing so each merge is near-trivial and history stays close to linear.
- Pair short-lived release branches with trunk-based only when you must support deployed versions; day-to-day work still happens on the trunk.

## Quick Refs

```bash
# Daily trunk-based loop
git switch main && git pull --rebase
git switch -c alice/small-change          # short-lived branch
# ...small commits...
git fetch && git rebase origin/main
git push -u origin alice/small-change
gh pr create --fill && gh pr merge --squash --delete-branch   # same day

# Feature flag (pseudo-config) to merge unfinished work safely
# if flags.isEnabled("new-checkout"): ...new path... else: ...old path...

# Keep branch tiny: split big work into stacked PRs
git switch -c alice/step-2 alice/step-1   # base each on the previous

# Bisect a regression introduced on a fast-moving trunk
...
|--------------|----------------|
| Fast CI | merges happen many times/day |
| Strong automated tests | broken trunk blocks everyone |
| Feature flags | merge incomplete work safely |
| Small PRs | quick review, fewer conflicts |
