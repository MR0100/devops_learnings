...
- Always create or move a branch before leaving a detached HEAD (`git switch -c <name>`); commits made detached are only reachable via reflog and expire.
- Use `git switch` for branch movement and `git checkout` only for files/paths — separating the verbs prevents accidental detached-HEAD checkouts.
- Push with explicit refspecs (`git push origin HEAD:<remote-branch>`) when names differ, and prefer `--force-with-lease` over `--force` to avoid clobbering others' work.
- Set up tracking once with `git push -u origin <branch>` so later `git pull`/`git push` need no arguments and report ahead/behind correctly.
- Prefer annotated tags for releases (`git tag -a v1.0 -m ...`) so they carry metadata and survive `git describe`; reserve lightweight tags for private bookmarks.
- Delete merged branches with lowercase `-d` (safe) and reserve `-D` for branches you have verified are gone or recoverable.

## Quick Refs

```bash
# Inspect HEAD and resolve refs
cat .git/HEAD                       # ref: refs/heads/main  OR raw SHA (detached)
git symbolic-ref HEAD               # which branch HEAD points to
git rev-parse --abbrev-ref HEAD     # current branch name
git rev-parse <ref>                 # ref → full SHA

# Ancestry navigation
git rev-parse HEAD~3                # 3rd parent back (first-parent chain)
git rev-parse HEAD^2                # 2nd parent of a merge commit
git rev-parse 'main@{2.days.ago}'   # where main pointed 2 days ago

# Branch / tag plumbing
git update-ref refs/heads/x <sha>   # move a branch without checkout
git for-each-ref --format='%(refname:short) %(objectname:short)'
git branch --contains <sha>         # which branches include a commit
git tag --points-at HEAD            # tags on current commit

# Special refs
git log -1 ORIG_HEAD                # pre-reset/merge position
cat .git/FETCH_HEAD                 # last fetched tips
```

| Symbol | Means |
|--------|-------|
| `HEAD` | current commit (via branch or detached SHA) |
| `<branch>~N` | Nth first-parent ancestor |
| `<branch>^N` | Nth parent of a merge commit |
| `@{N}` | Nth previous reflog position of that ref |
