...
- **Eyeballing `time_total` only** — it hides *where* the time went. Use the `-w` timing breakdown to split DNS vs TCP vs TLS vs server vs transfer.
- **Leaving `-k` in scripts** — `--insecure` disables cert validation; great for a quick test, a silent security hole if it ships to prod.
- **Editing `/etc/hosts` to test one backend** — use `--resolve host:port:IP` instead; it's scoped to the single command and won't be forgotten.
- **Forgetting `-L`** — without follow-redirects you debug the 301/302, not the real response; with it, remember each hop has its own timing.
- **Dropping `-s` in pipelines** — the progress meter writes to stderr and pollutes parsed output; pair `-s` with `-S` so real errors still surface.

## Best Practices

- Keep a **`curl-format.txt`** handy and alias a timing probe (`curl -w @curl-format.txt -o /dev/null -s URL`) — it's the fastest "is it DNS, TLS, or the server?" triage.
- Pin the endpoint with **`--resolve`** to test a specific origin/PoP behind a VIP without DNS changes or `/etc/hosts` edits.
- Force the **protocol/TLS version** (`--http1.1`, `--http2`, `--tlsv1.3`) to isolate negotiation bugs instead of guessing.
- For machine consumption, emit **`-w '%{json}'`** and pipe to `jq` — structured metrics beat scraping verbose text.
- Capture failures with **`-v` (or `--trace-ascii`)** so you have the full request/response and TLS exchange when filing a bug.

## Quick Refs

```bash
# Timing breakdown (the triage one-liner)
curl -w "DNS %{time_namelookup}  TCP %{time_connect}  TLS %{time_appconnect}  TTFB %{time_starttransfer}  total %{time_total}\n" \
     -o /dev/null -s https://example.com

# Hit a specific backend IP with the right Host/SNI
curl --resolve example.com:443:1.2.3.4 https://example.com/
...
# Force protocol / TLS version
curl --http2 --tlsv1.3 -v https://example.com
```

Phase → suspect: slow **`time_namelookup`** = DNS/resolver; slow **`time_connect`** = network/RTT/firewall; slow **`time_appconnect`** = TLS handshake (version, resumption); slow **`time_starttransfer`** = server processing; slow tail = bandwidth/body size.
