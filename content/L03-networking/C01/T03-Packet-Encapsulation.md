...
- **Leaving tunnel/overlay MTU at 1500**: VXLAN (-50), GRE (-24), IPsec (-50 to -100), and WireGuard (-60) all shrink the usable payload. Pods/VMs still advertising 1500 produce the classic "small requests fine, big ones hang" symptom.
- **Blocking ICMP "everywhere"**: PMTUD relies on ICMP type 3 code 4 (Fragmentation Needed). Drop it and oversized DF packets vanish into a black hole after the handshake succeeds.
- **Confusing MTU with MSS**: MTU is the L2 frame payload limit (1500); MSS is the TCP *segment* limit (MTU − 40 for IPv4, − 60 for IPv6). Tuning one without the other misses the mark.
- **Ignoring the IPv6 difference**: IPv6 routers never fragment — only the sender does, via PMTUD. A broken ICMPv6 path breaks IPv6 connectivity more abruptly than IPv4.
- **Forgetting framing overhead on tiny payloads**: A 50-byte DNS query carries ~46 bytes of UDP+IP+Ethernet framing — overhead, not payload — which matters at high query rates.

## Best Practices

- **Set MTU explicitly on every tunnel/overlay** and clamp TCP MSS at the edge (`iptables ... TCPMSS --clamp-mss-to-pmtu`, or your CNI's MTU setting) so endpoints negotiate a size that survives the path.
- **Always allow ICMP "Fragmentation Needed" (IPv4) and "Packet Too Big" (ICMPv6)** through firewalls; PMTUD is non-optional for correctness.
- **Use jumbo frames (MTU 9000) only on paths that support them end-to-end** — DC backbones, storage, intra-AZ. A single non-jumbo hop reintroduces fragmentation or drops.
- **Verify path MTU before blaming the app** when transfers stall mid-stream after a clean handshake — it's the signature of an MTU/ICMP black hole.
- **Account for header overhead in capacity math**: at 1500 MTU expect ~3-4% framing overhead for bulk transfers; design throughput budgets accordingly.

## Quick Refs

| Header | Size (bytes) |
|---|---|
| Ethernet (hdr + FCS) | 14 + 4 = 18 |
| 802.1Q VLAN tag | +4 |
| IPv4 | 20 (no options) |
| IPv6 | 40 (fixed) |
| TCP | 20 (no options) |
...
Rules of thumb: **MSS = MTU − 40** (IPv4+TCP), **−60** (IPv6+TCP). Ethernet payload ceiling = 1500.

```bash
ip link show eth0                       # current interface MTU
# Find path MTU: largest unfragmented ICMP payload (+28 = total IPv4 size)
ping -M do -s 1472 example.com          # 1472 + 28 = 1500; reduce until it passes
tracepath example.com                   # discover PMTU hop by hop
# Clamp TCP MSS to path MTU (router/edge)
