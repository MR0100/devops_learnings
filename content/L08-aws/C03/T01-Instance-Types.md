...
- Default to the latest generation; older generations cost more for less performance
- Try Graviton (`g` variant) first — typically 40-60% better perf/$ for recompilable Linux workloads
- Match the family to the bottleneck: M general, C for CPU, R for memory, I for NVMe IOPS
- Start small and right-size from real metrics; target ~40-60% average CPU at peak
- Use Compute Optimizer recommendations rather than eyeballing utilization
- Reserve burstable (T family) for genuinely spiky workloads; use `unlimited` mode only with cost awareness
- Diversify instance types and AZs for Spot fleets to reduce interruption rate
