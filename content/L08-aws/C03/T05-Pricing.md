...
- Blend the models: Savings Plans for steady baseline, on-demand for variable, Spot for interruption-tolerant work
- Prefer Compute Savings Plans over RIs for flexibility; reserve EC2 Instance SPs/Standard RIs for truly fixed footprints
- Commit ~70% of measured baseline, not peak — unused commitment is pure waste
- Track SP coverage and utilization in Cost Explorer; keep both above ~80%
- For Spot: diversify instance types and AZs, use price-capacity-optimized, and handle the 2-minute termination notice
- Right-size with Compute Optimizer before committing — don't lock in oversized instances for years
- Layer Cost Anomaly Detection to catch both spend spikes and rising Spot interruption rates
