...
```bash
# Start a Logs Insights query and fetch results
QUERY_ID=$(aws logs start-query --log-group-name /myapp \
  --start-time 1717584000 --end-time 1717587600 \
  --query-string 'fields @message | filter level = "ERROR" | stats count() by @message' \
  --query queryId --output text)
aws logs get-query-results --query-id "$QUERY_ID"

# Save a reusable query
aws logs put-query-definition --name top-errors --query-string 'fields @message | filter level="ERROR"'

# Stream matching logs out via subscription filter
aws logs put-subscription-filter --log-group-name /myapp/api --filter-name errors \
  --filter-pattern '{$.level = "ERROR"}' --destination-arn arn:aws:kinesis:...
```
