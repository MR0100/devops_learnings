...
- Keep functions small, single-purpose, and stateless; push all state to DynamoDB/S3/Redis and pass context explicitly.
- Initialize clients/connections at module scope so warm invocations reuse them, and front RDS with RDS Proxy to avoid connection exhaustion.
- Right-size memory (CPU scales with it) and measure cost per invocation; reserve provisioned concurrency only for latency-sensitive hot paths.
- Orchestrate multi-step work with Step Functions instead of chaining Lambda-to-Lambda invocations.
- Configure retries and a dead-letter queue for async sources, and emit structured logs with sane retention to control CloudWatch cost.
- Stay out of the VPC unless you need private resources (it slows cold starts), and use SnapStart (Java) or smaller packages to cut cold-start latency.

## Quick Refs

When serverless fits:

| Serverless wins | Serverless loses |
|---|---|
| Event-driven, bursty, sporadic | Sustained high traffic (EC2/ECS cheaper) |
| Glue/automation, webhooks | Long-running jobs (>15 min cap) |
| Low-traffic APIs, background jobs | Latency-critical (cold start), stateful, GPU |

Lambda hard limits: 15-min timeout · 10 GB RAM · 512 MB `/tmp` (10 GB via EFS) · 250 MB code (50 MB zipped) or 10 GB container image · 1000 default concurrency/region.

Pricing: $0.20 / 1M requests + $0.0000166667 / GB-second (e.g. 1M × 1s × 1GB ≈ $16.87).

```bash
# Local test
sam local invoke -e event.json MyFunction
# Publish a shared layer
aws lambda publish-layer-version --layer-name my-libs --zip-file fileb://layer.zip
```

Cold-start levers: provisioned concurrency · smaller package · avoid VPC · supported runtime · SnapStart (Java).
