...
- Treat configuration as your job: enable AWS Config/Security Hub (or equivalents) and codify guardrails so misconfigurations are caught automatically.
- Enforce least-privilege IAM and MFA, and rotate keys — IAM mistakes drive ~90% of cloud breaches.
- Block public S3 by default at the account level and require encryption via Service Control Policies/org guardrails.
- Turn on and retain audit logs (CloudTrail, VPC Flow Logs) and route them to a separate, locked-down account.
- Design DR yourself using provider primitives (CRR, cross-region replicas, Route 53 failover); multi-AZ is not multi-region.
- Use separate accounts per env/business unit to limit blast radius, and test backups/restores — don't trust an untested DR plan.
...
| Service | Provider patches | You patch |
|---|---|---|
| EC2 OS | No | OS, packages |
| RDS engine | Yes (maint window) | Schema/queries |
| Lambda runtime | Yes | Function code |
| EKS control plane | Yes | Node AMI (you trigger) |
| Fargate | Yes | App image |

Mental model: provider owns security **OF** the cloud (hardware, hypervisor, physical, network backbone); you own security **IN** the cloud (data, IAM, config, OS in IaaS, app). Higher abstraction (IaaS → PaaS → FaaS → SaaS) shifts more onto the provider.

Top customer-side breach causes to audit first: public S3 buckets, permissive IAM roles, security groups open to `0.0.0.0/0` on SSH/RDP, exposed DB endpoints, hard-coded keys in code.
