...
- Grant least privilege: specific actions and resource ARNs (not `*`), tightened with conditions; iterate using Access Analyzer and CloudTrail "last accessed" data.
- Use roles and temporary STS credentials for all workloads (EC2 instance profiles, Lambda roles, IRSA on EKS) — never static access keys on servers.
- Federate human access via Identity Center/SSO (SAML/OIDC) and avoid long-lived IAM users.
- Layer guardrails: SCPs at the org/OU level and permissions boundaries on delegated roles so a mistake can't exceed the allowed maximum.
- Require MFA and add an `ExternalId` (plus tight trust policies) on cross-account `AssumeRole`.
- Audit continuously: CloudTrail for every API call, Access Analyzer for external sharing, and prune unused permissions monthly.
...
1. Explicit `Deny` anywhere → DENY
2. Allow in identity (+resource) policy → ALLOW
3. Allow in identity only (same account) → ALLOW
4. Allow in resource only (cross-account) → ALLOW
5. No matching Allow → implicit DENY

Guardrail comparison:

| | SCP | Permissions Boundary | Identity/Resource Policy |
|---|---|---|---|
| Scope | OU/Account | Role/User | Role/User/Resource |
| Set by | Org admin | Account admin | IAM admin |
| Effect | Max allowed | Max allowed | Grants permission |

ARN shape: `arn:aws:service:region:account:resource`. Policy statement = Effect + Action + Resource + (optional) Condition, evaluated against a Principal.

```bash
aws sts assume-role --role-arn arn:aws:iam::222:role/Y --role-session-name me
aws sso login   # human CLI access
```
