...
- Plan non-overlapping CIDRs up front (use VPC IPAM) — overlap permanently blocks peering and VPN.
- Use a 3-tier layout: load balancers/bastion in public subnets, app and DB tiers in private subnets across multiple AZs.
- Keep databases off the public internet and prefer SSM Session Manager over inbound SSH; never open critical ports to `0.0.0.0/0`.
- Deploy one NAT Gateway per AZ for HA, and cut NAT cost with Gateway endpoints (S3/DynamoDB, free) and interface endpoints for high-traffic services.
- Lean on stateful security groups (referencing other SGs) as the primary firewall; reserve NACLs for coarse subnet-level deny rules.
- Enable VPC Flow Logs for audit/debugging/cost analysis and use Reachability Analyzer to diagnose connectivity instead of guessing.

## Quick Refs

CIDR sizing: VPC /16 (65k IPs) or /20 (4k); subnets typically /24 (256 IPs, minus 5 reserved). Leave room for future AZs/tiers.

Public vs private subnet = route table: `0.0.0.0/0 → igw-xxx` (public) vs `0.0.0.0/0 → nat-xxx` (private).

Connectivity options:

| Need | Use |
|---|---|
| VPC ↔ VPC (few) | Peering (non-transitive) |
| Many VPCs + on-prem | Transit Gateway (transitive hub) |
| On-prem over internet | Site-to-site VPN (~1.25 Gbps/tunnel) |
| Dedicated on-prem link | Direct Connect (1–100 Gbps) |
| Reach AWS services privately | VPC endpoints (gateway = free, interface = $/hr+GB) |

NAT Gateway cost: ~$0.045/hr + ~$0.045/GB processed — frequently the biggest networking line item.
