...
- Default to single-region multi-AZ; add cross-region only when a real driver exists (tier-0 RTO, global latency, compliance, scale beyond one region).
- Match the DR pattern to explicit RTO/RPO targets (backup-restore → pilot light → warm standby → active-active) and pay only for the resilience you need.
- Design data placement to minimize cross-region round-trips: read locally, write to a primary, and avoid chatty inter-region calls in loops.
- For multi-region writes use a database built for it (DynamoDB Global Tables, Spanner, CockroachDB) or shard users by region — never bolt active-active onto a last-writer-wins store without a conflict plan.
- Use CDN/edge caching and Global Accelerator (or GCP global LB) to cut inter-region traffic and speed failover beyond DNS.
- Test failover regularly and monitor replication lag and per-service cross-region transfer cost.
...
|---|---|---|---|
| Backup-Restore | Hours | Backup interval | Lowest |
| Pilot Light | 10s of min | Minutes | Low |
| Warm Standby | Minutes | Seconds | Medium |
| Active-Active | Seconds | ~0 | Highest |

Cross-region replication primitives: S3 CRR (async, RPO minutes) · DynamoDB Global Tables (active-active, RPO seconds) · Aurora Global Database (<1s RPO) · RDS cross-region read replica (manual promote).

Latency floor: same-AZ <1ms · cross-AZ 1–2ms · US↔US ~70ms · US↔EU ~80–100ms · US↔Asia ~150–200ms. Transfer cost: cross-AZ $0.01/GB · cross-region $0.02–0.09/GB · internet egress $0.09/GB (1 TB/day cross-region ≈ $7k–32k/yr).
