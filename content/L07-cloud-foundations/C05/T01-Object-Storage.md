...
Picking an S3 storage class:

| Access pattern | Class |
|---|---|
| Hot, frequent | Standard |
| Infrequent, must be instant | Standard-IA (One Zone-IA if re-creatable) |
| Unknown / variable | Intelligent-Tiering (auto-moves; no retrieval fees) |
| Archive, occasional ms reads | Glacier Instant Retrieval |
| Archive, retrieval in minutes-hours OK | Glacier Flexible |
| Cold archive, 12h+ retrieval OK | Glacier Deep Archive (cheapest) |

Watch out: IA/Glacier classes carry **per-GB retrieval fees** and **minimum storage durations** (30/90/180 days) — frequent access or early deletion can cost more than Standard.

```bash
# Common CLI
aws s3 cp file s3://bucket/key --storage-class STANDARD_IA
aws s3 ls s3://bucket/prefix/ --recursive --human-readable --summarize
aws s3api put-bucket-versioning --bucket b --versioning-configuration Status=Enabled
aws s3 sync ./dir s3://bucket/prefix/ --delete   # mirror local → bucket
```
