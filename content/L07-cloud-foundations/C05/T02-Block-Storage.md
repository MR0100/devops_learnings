...
- Default to gp3 and tune IOPS/throughput independently; only step up to io2/io2 Block Express for latency-critical, high-IOPS databases.
- Encrypt volumes with KMS (turn on account-level default encryption) and copy encrypted snapshots to a second region for DR.
- Automate snapshots with Data Lifecycle Manager (daily, retain N, cross-region copy) rather than relying on manual backups.
- Use instance store only for cache/scratch/ephemeral data — it is lost on stop/terminate.
- Always unmount cleanly before detaching, and grow volumes online (`modify-volume` then `resize2fs`/`xfs_growfs`); you cannot shrink in place.
- Monitor `VolumeQueueLength` and IOPS/throughput; alert on saturation and right-size before over-provisioning IOPS.

## Quick Refs

EBS volume type selector:

| Need | Type |
|---|---|
| General default | gp3 |
| Latency-critical / high-IOPS DB | io2 / io2 Block Express |
| Big sequential (logs, big data) | st1 (throughput HDD) |
| Cold, infrequent | sc1 (cold HDD) |
| Microsecond local scratch | Instance store (ephemeral) |

Block vs object vs file:

| | Block (EBS) | Object (S3) | File (EFS) |
|---|---|---|---|
| Interface | Raw blocks | HTTP API | NFS/SMB |
| Mounts | 1 host | No | Many hosts |
| Latency | sub-ms | ms | ms |
| Best for | DB, OS disk | Backup, data lake | Shared FS |

```bash
aws ec2 create-snapshot --volume-id vol-xxx
aws ec2 modify-volume --volume-id vol-xxx --size 100   # grow, then resize FS
aws ec2 attach-volume --volume-id vol-xxx --instance-id i-xxx --device /dev/sdf
```
