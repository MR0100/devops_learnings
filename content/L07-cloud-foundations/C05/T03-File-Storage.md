...
- Reach for file storage only when many hosts must share a POSIX filesystem; default to S3 (or a DB/Redis) when the app can use it.
- Use EFS lifecycle management to auto-transition cold data to IA tiers and cut cost dramatically (e.g. $30 → ~$2.50/mo per 100 GB).
- Enforce per-application access with EFS Access Points (UID/GID/path/quota) and restrict mount targets via security groups; layer IAM auth where available.
- Tune NFS mounts (large `rsize`/`wsize`, `hard`, sequential I/O) and cache hot reads locally — many small files are slow over NFS.
- Pick FSx for Lustre for HPC/ML training throughput and `ReadWriteMany` EFS volumes for multi-pod EKS workloads; never put a database on file storage.
- Back up with AWS Backup and monitor `BurstCreditBalance`/`PercentIOLimit`, alerting before credits hit zero.
...
| Situation | Use |
|---|---|
| Single host disk | EBS (block) |
| App can decouple read/write | S3 (object) |
| Many hosts share a POSIX FS | EFS / Filestore / Azure Files |
| HPC / ML high-throughput parallel | FSx for Lustre |
| Database | Block (EBS) or managed DB — never file |

EFS vs EBS at a glance: EFS = NFS, many hosts, `ReadWriteMany`, ms latency, scales automatically; EBS = block, one host, sub-ms latency, fixed size. EFS is slower and pricier per GB at Standard tier — use IA tiers for cold data.

```bash
sudo mount -t nfs4 fs-xxxx.efs.us-east-1.amazonaws.com:/ /mnt/efs
# recommended NFS opts
