...
```python
# Consumer: verify HMAC signature, ack fast, process async
import hmac, hashlib
...
async def receive(request: Request, bg: BackgroundTasks,
    ...
    if already_processed(event["id"]):        # idempotent on event id
        return {"status": "duplicate"}
    bg.add_task(handle_event, event)          # ack now, work later
    return {"status": "queued"}               # 200 fast (<5s)
```

```text
RESPONSE CODE SEMANTICS (consumer -> provider)
  2xx  delivered, do not retry
  4xx  permanent failure (bad signature/payload) -> provider stops
  5xx / timeout  transient -> provider RETRIES with backoff
```

```bash
# Local dev: expose localhost so providers can POST
ngrok http 3000          # -> https://<id>.ngrok.io -> localhost:3000
```

```json
// CloudEvents 1.0 envelope (Knative, Argo Events, EventBridge)
{ "specversion": "1.0", "type": "com.example.user.created",
  "source": "/users", "id": "abc123", "time": "2026-06-09T10:00:00Z",
  "data": { "userId": 42 } }
```
