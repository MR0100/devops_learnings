...
- Using the root account (or root access keys) for day-to-day work instead of an IAM/SSO identity
- No MFA on root — or only virtual MFA with no hardware backup, so a lost phone locks you out
- Discovering billing only after the invoice arrives: no budget, no CloudWatch billing alarm, no Cost Anomaly Detection
- Enabling CloudTrail in one region only, so activity in other regions is invisible
- Root email tied to one employee's mailbox — account recovery breaks when they leave
- Treating "harden it later" as acceptable; permissive SGs and broad IAM grants ossify quickly

## Best Practices

- Hardware MFA on root, credentials stored offline, root used only for the handful of root-only tasks
- Daily access via IAM Identity Center (SSO), never the root user
- Multi-region CloudTrail with log-file validation and KMS encryption from day one
- Layered cost guardrails: monthly Budgets, a CloudWatch billing alarm wired to SNS, and Cost Anomaly Detection
- Catchall/role-based root email (e.g. `aws+prod@company.com`), not a personal inbox
- Block unused regions with an SCP and request quota increases proactively for known needs
