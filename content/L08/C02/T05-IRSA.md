...
# Create role + ServiceAccount + annotation in one step (IRSA)
eksctl create iamserviceaccount --cluster mycluster --namespace my-ns --name my-sa \
  --attach-policy-arn arn:aws:iam::aws:policy/AmazonS3ReadOnlyAccess --approve

# EKS Pod Identity association (newer alternative)
aws eks create-pod-identity-association --cluster-name mycluster --namespace my-ns \
  --service-account my-sa --role-arn arn:aws:iam::123:role/MyAppRole

# Verify injected env in a running pod
