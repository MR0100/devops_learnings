...
- **Define "deploy" precisely** — count distinct changes reaching production, not pipeline runs; write the definition down so it's comparable over time.
- **Report DF with CFR** — frequency in isolation invites gaming; the pair shows whether speed is safe.
- **Segment by team and service** — surface the teams in the basement instead of hiding them under an org-wide average.
- **Shrink the batch, don't pad the count** — drive DF up via trunk-based development, small PRs, and feature flags, not by splitting one release into ten cosmetic ones.
- **Decouple deploy from release** — flags let you deploy continuously while controlling exposure, raising DF without raising risk.

## Quick Refs

**What counts as a deploy?** A distinct change reaching production users. Re-running the same pipeline does not. Config-only changes do.

**Drivers of high DF**

| Lever | Effect |
|---|---|
| Trunk-based development | Removes long-lived branch merge pain |
| CI under ~10 min | Keeps the feedback loop tight |
| Feature flags | Decouples deploy from release |
| Automated tests + rollback | Makes frequent deploys safe |

**DORA 2023 benchmarks**: Elite = on-demand (multiple/day) · High = daily–weekly · Medium = weekly–monthly · Low = monthly+. Always read DF *next to* CFR.
