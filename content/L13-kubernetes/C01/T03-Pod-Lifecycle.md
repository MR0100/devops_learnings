...
Native sidecars (init containers with `restartPolicy: Always`): alpha 1.28, on-by-default beta 1.29, GA 1.33.
...
# Logs (current / previous instance / specific container)
kubectl logs <pod>
kubectl logs <pod> --previous
kubectl logs <pod> -c <container> -f

# Live debug on a running/crashing pod
kubectl debug -it <pod> --image=busybox --target=<container>

# Force-ish delete (after grace period)
kubectl delete pod <pod> --grace-period=30
```

| Phase | Meaning |
|---|---|
| Pending | Accepted, not yet running (scheduling, image pull, volume) |
| Running | Bound to node, ≥1 container started |
| Succeeded | All containers exited 0, no restart |
| Failed | ≥1 container exited non-zero (terminal) |
| Unknown | Node communication lost |

**Termination order**: deletionTimestamp set → removed from endpoints → preStop hook → SIGTERM → `terminationGracePeriodSeconds` countdown → SIGKILL → volumes unmounted → removed from etcd.
