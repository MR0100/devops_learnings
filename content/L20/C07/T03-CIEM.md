...
**Junior**: "What is CIEM?" — Cloud Infrastructure Entitlement Management analyzes cloud identities and their permissions to find excessive or unused entitlements and help enforce least privilege.

**Mid**: "How does CIEM figure out what permissions to remove?" — It compares granted permissions against permissions actually used (from access logs like CloudTrail) and flags the gap, so you can right-size roles to what they really exercise.

**Senior**: "How do you safely reduce over-permissioned roles in production?" — Use observed-usage data to propose a least-privilege policy, roll it out in a monitor/dry-run mode first to catch denied calls, then enforce — never strip permissions blindly, since missed dependencies cause outages.

**Staff**: "How do you manage identity posture at scale across multi-cloud?" — Treat entitlements as continuously-governed: CIEM-driven right-sizing feeding back into IaC-defined roles, JIT elevation for privileged access, guardrails (SCPs/permission boundaries) capping the blast radius, and correlation with posture/workload findings so an over-permissioned identity on an exposed resource is prioritized as a real attack path.
