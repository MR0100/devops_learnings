...
- **Grant the narrowest capability, not root** — `cap_net_bind_service` to bind low ports, `cap_net_admin` for network config; never reach for setuid root when a capability fits.
- **Prefer group ownership + `setgid` over loose mode bits** for shared directories so new files inherit the correct group automatically.
- **Drop all capabilities, then add back** — in containers run `--cap-drop=ALL` and re-add only what the workload needs (`--cap-add`), mirroring K8s `securityContext.capabilities`.
- **Use ACLs for the exceptions, base perms for the rule** — keep the owner/group/other model as the baseline and reach for `setfacl` only when a few extra principals need access.
- **Audit for the danger trio** — periodically scan for `chmod 777`, stray setuid-root binaries (`find / -perm -4000`), and world-writable files.
...
chmod o-w,a+x file          # symbolic edits
umask 027                   # new files 640, new dirs 750

# POSIX ACLs
getfacl file                # view ACL
setfacl -m u:deploy:rx file # grant user deploy read+execute
setfacl -d -m g:web:rwx dir # default ACL for new entries
setfacl -b file             # remove all ACLs

# Capabilities
getcap /usr/bin/ping        # show file caps
setcap cap_net_bind_service+ep ./server   # bind <1024 without root
capsh --print               # show current process caps
```

**Octal cheat**: r=4, w=2, x=1 → `7=rwx 6=rw- 5=r-x 4=r-- 0=---`. A trailing `+` on `ls -l` (e.g. `-rw-r--r--+`) means an ACL is present.
