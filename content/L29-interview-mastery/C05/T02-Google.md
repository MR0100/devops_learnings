...
Google originated SRE, so for SRE roles expect depth on: SLIs/SLOs/error budgets, toil reduction, capacity planning, incident response, and the production mindset from the SRE book. A DevOps candidate who can speak fluently about error budgets and NALSD has a real edge here (see [L19](../../L19-sre/README.md)).
