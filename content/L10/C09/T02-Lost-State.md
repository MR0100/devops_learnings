...
- **No S3 versioning** on the state bucket — a single delete or overwrite is unrecoverable.
- **`terraform apply` after state loss** — plan shows "N to add"; applying duplicates live resources. Stop and recover/import first.
- **`force-unlock` while an apply is genuinely running** — corrupts state. Only unlock when certain no apply is in flight.
- **Editing or pushing state without a backup** — `state push`/manual JSON edits with no `state pull` snapshot first.
- **Single huge state for everything** — one corruption or bad surgery blasts the whole estate; split per workload.
- **Untested recovery** — versioning and replication that nobody has ever restored from; assume it doesn't work until drilled.
- **No DynamoDB lock** — concurrent applies race and corrupt state.
