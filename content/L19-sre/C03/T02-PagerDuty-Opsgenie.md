...
**Junior**: "What does a pager system like PagerDuty do?" — It receives alerts from sources like Alertmanager or Datadog, maps them to a service with an escalation policy and schedule, and notifies the on-call engineer via push/SMS/call until someone acknowledges.

**Mid**: "What are the core PagerDuty building blocks?" — Service (what to page on), escalation policy (who to page next and when), schedule (which engineer is on at a given time), override (vacation/swaps), and integration (the inbound alert source).

**Senior**: "How do you design an escalation policy that guarantees response without over-paging?" — Layer it with ack timeouts — primary at 5 min, secondary at ~10, manager beyond that — route only customer-impacting severities to a page while warnings go to tickets/email, and use auto-resolution and maintenance windows so transient or expected alerts never wake anyone.

**Staff**: "How do you set up incident routing across many services?" — Define one service per microservice with team-specific escalation policies, route alerts by label (e.g. team=payments → payments rotation) in Alertmanager so the right people are paged directly, and feed the platform's audit timeline (ack times, resolution times) into postmortems to drive continuous improvement.
