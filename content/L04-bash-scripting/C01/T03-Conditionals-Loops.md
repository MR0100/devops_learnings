...
- **`[ $x = 1 ]` with an empty/unset `$x`**: collapses to `[ = 1 ]` → syntax error. Quote it: `[ "$x" = 1 ]`, or use `[[ ]]`.
- **`=` vs `-eq`**: `[ "$a" = "$b" ]` is string compare, `[ "$a" -eq "$b" ]` is numeric. `[ 01 = 1 ]` is false but `[ 01 -eq 1 ]` is true.
- **`for f in $(ls)`**: word-splits and globs filenames. Use `for f in *.log` (glob directly) or `while IFS= read -r`.
- **`echo` to "return" a string from a function but forgetting it goes to stdout**: any stray `echo`/log inside the function corrupts the captured value. Send logs to `>&2`.
- **`while read` after a pipe**: the loop runs in a subshell, so counters/arrays set inside vanish. Feed it with `< <(cmd)` (process substitution) instead.
- **Dropping `local`**: an unguarded `x=...` inside a function clobbers a caller variable of the same name.

## Best Practices

- Prefer `[[ ]]` for tests (no word-splitting, supports `=~`, `&&`, glob matching) and `(( ))` for arithmetic comparisons.
- Read files with `while IFS= read -r line; do ...; done < file` to preserve whitespace and backslashes.
- Always `local` your function variables and return status via exit code (`return`/bare test), reserving stdout for real output.
- Guard loop bodies early with `[[ cond ]] || continue` instead of deep nesting.
- Use `case` over long `if/elif` ladders when matching one value against many patterns.
- Validate inputs up front: `[[ $# -eq 2 ]] || { usage; exit 2; }`.

## Quick Refs

```bash
# Test operators
[[ -f path ]]   # regular file       [[ -d path ]]   # directory
[[ -e path ]]   # exists (any type)  [[ -s path ]]   # exists & non-empty
[[ -z "$s" ]]   # empty string       [[ -n "$s" ]]   # non-empty
[[ "$a" == "$b" ]]   [[ "$a" != "$b" ]]   [[ "$s" == *.log ]]   # glob
[[ "$s" =~ ^[0-9]+$ ]]                                   # regex
[[ "$a" -eq "$b" ]]  -ne -lt -le -gt -ge                # numeric (in [ ])
(( a > b )) && echo yes                                  # arithmetic test

# Loops
for i in {1..5}; do echo "$i"; done
for ((i=0; i<5; i++)); do echo "$i"; done
for f in ./*.txt; do [[ -e "$f" ]] || continue; echo "$f"; done
while IFS= read -r line; do echo "$line"; done < file
until ping -c1 -W1 host &>/dev/null; do sleep 1; done

# Read command output without a subshell
while IFS= read -r f; do ((n++)); done < <(find . -type f)
