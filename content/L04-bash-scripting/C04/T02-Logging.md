...
- Send all logs to stderr (`>&2`) and reserve stdout for the script's actual data output, so pipelines like `script | jq` stay clean.
- Timestamp every line (prefer UTC ISO-8601) and tag a level (`INFO`/`WARN`/`ERROR`) so logs are correlatable and filterable.
- Gate verbosity with a `LOG_LEVEL` env var rather than commenting lines in and out.
- Detect a TTY with `[[ -t 2 ]]` before emitting ANSI colors so log files and pipes stay free of escape codes.
- Emit structured JSON when shipping to an aggregator (Loki/ELK/CloudWatch); keep human-readable text for interactive runs.
- Never log secrets — redact tokens, passwords, and keys before they hit the log; `set -x` traces also leak them.
- Mirror to a file with `exec > >(tee -a "$LOG_FILE") 2>&1` when you need both terminal and persistent output.

## Quick Refs

```bash
# Stream basics
echo "data"            # stdout = real output
echo "[INFO] ..." >&2  # stderr = logs

# Redirection
cmd >out.txt           # stdout to file
cmd 2>err.txt          # stderr to file
cmd >out 2>&1          # both to out (order matters!)
cmd &>all.log          # bash shorthand for both
cmd >/dev/null 2>&1    # discard everything

# Logging helpers
log()   { printf '%(%Y-%m-%dT%H:%M:%SZ)T %s\n' -1 "$*" >&2; }   # bash 4.2+, UTC needs TZ=UTC
warn()  { echo "[WARN]  $*" >&2; }
err()   { echo "[ERROR] $*" >&2; }
fatal() { err "$*"; exit 1; }

# Color only on a terminal
[[ -t 2 ]] && { RED=$'\e[31m'; RST=$'\e[0m'; } || { RED=; RST=; }
echo "${RED}error${RST}" >&2

# Structured JSON line
jlog() { jq -nc --arg ts "$(date -uIs)" --arg lvl "$1" --arg msg "$2" \
  '{ts:$ts, level:$lvl, msg:$msg}' >&2; }

# Tee everything to a file from here on
exec > >(tee -a "/var/log/$(basename "$0" .sh).log") 2>&1

# syslog / journald
logger -t myscript -p user.warn "disk filling up"
```
