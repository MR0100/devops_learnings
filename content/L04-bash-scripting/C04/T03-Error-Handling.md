...
- Open with `set -Eeuo pipefail` and an `ERR` trap that reports the failing line, function, and exit code; pair with an `EXIT` trap for cleanup.
- Distinguish recoverable from fatal: continue past per-item failures and report at the end, but abort immediately on a corrupted-state error (e.g., backup failed).
- Wrap flaky network/IO calls in a retry-with-backoff helper, and cap with `timeout` so nothing hangs forever.
- Make destructive paths reversible: snapshot before mutating and restore on failure; guard truly destructive actions behind confirmation or `--force`.
- Surface actionable error messages (what failed, on what input, what to do) instead of generic "command failed".
- Don't let `set -e` lull you — add explicit `|| { err ...; exit N; }` checks in conditional contexts and around command substitutions where `-e` is suspended.
...
# Strict mode + traps
set -Eeuo pipefail
trap 'rc=$?; echo "ERR ${BASH_SOURCE[0]}:$LINENO in ${FUNCNAME[0]:-main} (rc=$rc)" >&2' ERR
trap cleanup EXIT

# Per-command handling
cmd || { err "cmd failed"; exit 1; }
if ! out=$(cmd 2>&1); then err "failed: $out"; exit 1; fi

# Collect output AND status safely (local masks the code)
local result; result=$(maybe_fail) || rc=$?
...
retry() {  # retry MAX DELAY -- cmd...
  ...
  local n=0
  until "$@"; do
    (( ++n >= max )) && { err "gave up after $max"; return 1; }
    sleep "$delay"; delay=$((delay * 2))
  done
}
retry 5 1 curl -fsS https://api.example.com

# Timeout (returns 124 on timeout)
timeout 30 long_cmd || err "timed out"

# Aggregate failures across a loop
failed=()
for x in "${items[@]}"; do process "$x" || failed+=("$x"); done
(( ${#failed[@]} )) && { err "failed: ${failed[*]}"; exit 2; }

# Atomic, reversible mutation
cp -p "$target" "$target.bak"
modify "$target" || { mv "$target.bak" "$target"; exit 1; }
rm -f "$target.bak"
```
