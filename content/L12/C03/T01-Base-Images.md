...
- Choosing Alpine for a glibc app, then chasing `libc.so.6` / musl-vs-glibc breakage (DNS, locales, native wheels) in production.
- Treating `scratch`/distroless as drop-in and shipping without CA certs, `/etc/passwd`, or tzdata — TLS calls and time handling then fail at runtime, not build time.
- Optimizing only for image size and ignoring CVE posture — a tiny base with an unpatched runtime is not "secure."
- Picking distroless but still needing a shell to debug, instead of using the `:debug` variant or an ephemeral debug container.
- Conflating build-time and run-time needs — pulling a full SDK base for the final stage when a multi-stage runtime base would do.
- Standardizing org-wide on a base nobody patches, so the "golden" image quietly accumulates CVEs.
