...
Cookie flags: `HttpOnly` (no JS access) · `Secure` (HTTPS only) · `SameSite=Strict|Lax|None` (`None` requires `Secure`) · `Domain`/`Path` scope · `Max-Age`/`Expires` · `Partitioned` (CHIPS).

**Origin** = scheme + host + port (all three must match). Same-Origin Policy blocks JS from *reading* cross-origin responses, but does **not** block *sending* requests or loading img/script/css — which is why CSRF exists and why servers must still enforce authz (CORS is browser-side only).

Key CORS headers: `Access-Control-Allow-Origin` (specific origin, never `*` with credentials) · `-Allow-Methods` · `-Allow-Headers` · `-Allow-Credentials: true` · `-Max-Age` (cache preflight) · `Vary: Origin`. Preflight = `OPTIONS` for non-simple requests; respond `204`.

```bash
# Simulate a CORS preflight and read the response headers
curl -i -X OPTIONS https://api.example.com/foo \
  -H 'Origin: https://app.example.com' \
  -H 'Access-Control-Request-Method: POST' \
  -H 'Access-Control-Request-Headers: X-Custom'

# Inspect Set-Cookie flags
curl -sI https://example.com/login | grep -i set-cookie
```

Defenses: XSS → `HttpOnly` + CSP + SRI · CSRF → `SameSite` + CSRF tokens (CORS does **not** stop CSRF) · downgrade → `Secure` + HSTS.
