...
- **Building sticky sessions on anycast** — anycast has no session affinity; consecutive requests from one client can land on different PoPs. Keep PoPs stateless or use a shared backing store.
- **Assuming TCP will break** — in practice routes are stable (5-tuple ECMP) and most flows survive; over-engineering around imaginary mid-flow breakage is wasted effort.
- **Running your own anycast prematurely** — it needs an ASN, PI/PA addresses, BGP at multiple PoPs, and multi-ISP transit. For nearly everyone a managed layer is cheaper and safer.
- **Expecting load-based routing** — anycast routes by *network distance*, not load or health, unless you actively withdraw a PoP; it's not a load balancer.
- **Failing to withdraw a sick PoP** — if a PoP is unhealthy but keeps announcing, clients keep getting routed to it. Health checks must drive BGP withdrawal.

## Best Practices

- **Use a managed anycast service** — CDN edge IPs, public-resolver-style anycast, or **AWS Global Accelerator** (2 static anycast IPs, no ASN required) — unless you have a hard reason to run your own.
- Keep edge logic **stateless**; push shared state to a regional/global store so any PoP can serve any request.
- **Withdraw automatically on failure** — wire health checks to pull the BGP announcement from a degraded PoP; convergence in seconds beats DNS-TTL failover.
- Exploit anycast for **DDoS absorption**: attack volume spreads across all PoPs, so no single site is overwhelmed (provider capacity permitting).
- For HA, **advertise two distinct anycast IPs** (e.g., Global Accelerator's pair) and publish both so clients can fail over if one is unreachable.
...
| | Anycast | Unicast |
|---|---|---|
| IP→location | one IP, many PoPs | one IP, one location |
| Routing picks | nearest PoP (BGP) | the single host |
| Failover | withdraw announcement (seconds) | DNS change (TTL-bound) |
| DDoS | spread across PoPs | concentrated on one IP |
| Stateful TCP | usually fine, rare reconnect | always fine |

Protocol fit: **UDP/DNS/QUIC = ideal** (stateless / migration-aware) · **long-lived TCP = generally fine**.

```bash
# Which PoP does my anycast IP route to?
mtr -n 1.1.1.1

# Cloudflare trace (PoP code + datacenter)
curl -s http://1.1.1.1/cdn-cgi/trace | grep -E '^(colo|loc)='

# Compare PoPs from different vantage points
ssh tokyo-vps  "curl -s http://1.1.1.1/cdn-cgi/trace | grep ^colo"
ssh london-vps "curl -s http://1.1.1.1/cdn-cgi/trace | grep ^colo"

# Who owns / announces the IP
whois 1.1.1.1 | grep -iE 'origin|orgname'
```
