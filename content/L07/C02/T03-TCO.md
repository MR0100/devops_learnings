...
- Compute TCO end-to-end: not just compute/storage but egress, managed-service fees, support tier, and people (SREs/oncall) on both sides.
- Always include the on-prem ops tail (power, cooling, space, refresh, NOC staff) when comparing — hardware is only ~30% of real on-prem cost.
- Express cost as a unit metric ($/customer/month, $/1M requests) and drive it down over time rather than chasing absolute bill size.
- Reserve the baseline, scale the rest, and tier cold storage — governance is what makes cloud cheaper than a naive lift-and-shift.
- Re-run build-vs-buy (e.g. Datadog vs self-hosted observability) at your actual scale; the answer flips as you grow.
- Tag everything and review per-team spend monthly so cost is owned by the teams that create it.

## Quick Refs

TCO components to never forget:

| Bucket | Watch for |
|---|---|
| Compute | Instance hours + OS/DB licenses |
| Storage | $/GB + IOPS tier + snapshots/backups |
| Network | Egress, cross-AZ/region, NAT, LB hours |
| Managed services | RDS, Lambda, KMS ($1/key/mo), Secrets ($0.40/secret/mo) |
| Operations | DevOps/SRE staff, 24/7 oncall, support tier (3–10% of bill) |
| Indirect | Training, compliance audits, DR drills, tooling |

Cloud-vs-own decision rule:

- New product, variable load, small team, global reach → **cloud wins**
- Steady huge-scale 24/7, sunk DC, heavy egress, latency/sovereignty constraints → **owned can win** (cf. Dropbox saving ~$75M/yr off S3)

Egress quick math: 10 TB/mo to internet ≈ $900/mo; 1 PB/mo ≈ ~$1M/yr. Put a CDN in front of heavy outbound traffic.
