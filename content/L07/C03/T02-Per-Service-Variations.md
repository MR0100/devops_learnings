...
- Assuming all "managed" services manage the same things — EKS leaves you owning nodes/RBAC/networking, while Fargate/Lambda manage almost everything; the split differs per service.
- Choosing a lower-abstraction service (EC2/ECS-on-EC2) and then carrying OS-patching and scaling toil you didn't need to take on.
- Picking a proprietary, deeply locked-in service (Lambda + DynamoDB) without acknowledging the portability cost — fine when intentional, a trap when accidental.
- Building a "portable" architecture on one cloud's proprietary services — the worst of both worlds (lock-in plus extra work).
- Forgetting that backup retention, encryption-key choice, and HA configuration are *yours* even on managed DBs (RDS, DynamoDB, ElastiCache).
- Selecting a service that isn't in compliance scope (HIPAA/PCI/FedRAMP) and discovering it during an audit.

## Best Practices

- For each component pick the farthest-right service on the control↔managed spectrum (EC2 → ECS → EKS → Fargate → Lambda → SaaS) that still meets your control and compliance needs.
- Decide build-vs-buy and managed-vs-self-host explicitly per component; default to managed in cloud unless a concrete requirement forbids it.
- Match lock-in tolerance to the workload: low-risk glue can be cloud-native; core/portable workloads stay on containers and open formats.
- Confirm service compliance scope (AWS Artifact / Azure / GCP docs) before building regulated workloads.
- Read the SLA per service and design above it (defense in depth) rather than trusting a single component's uptime number.

## Quick Refs

Control vs work spectrum — pick the farthest right that fits:

```
More control  ← EC2 → ECS-on-EC2 → EKS → Fargate → Lambda → SaaS →  Less work
```

Lock-in by abstraction:

| Layer | Lock-in | Portability lever |
|---|---|---|
| IaaS (VMs) | Low | Standard OS images |
| CaaS (K8s) | Medium | Kubernetes abstracts the cloud |
| PaaS | High | App uses platform APIs |
| FaaS | Highest | Functions bind to cloud events |

Per-component decision questions: (1) Do I need to control the OS/engine version? (2) Who is oncall for it? (3) What's the lock-in cost? (4) Is it in compliance scope?
