...
- Use ALIAS/ANAME (not CNAME) at the zone apex, and point ALIAS records at AWS resources to avoid an extra DNS lookup.
- Keep TTLs short (~60s) on records you may need to fail over quickly, and longer on stable records to cut query volume.
- Send correct cache headers from the origin (`public, max-age=…, immutable` for hashed assets; `private, no-store` for user data) — the CDN only caches what the origin permits.
- Version asset URLs (`/static/v123/app.js`) instead of relying on slow CDN invalidations.
- Tune the cache key (strip irrelevant query strings/headers) so responses are actually shareable; a per-user cache key never caches.
- Pair the CDN with health-checked, latency- or failover-routing DNS, and rate-limit the origin so cache misses don't overwhelm it.
...
| Weighted | Canary / gradual split (70/30) |
| Latency | Lowest-latency region |
| Failover | Primary + health-checked secondary |
| Geo / Geoproximity | Route by user location/compliance |
| Multi-Value | Return multiple healthy IPs |

Cache-Control cheat sheet: hashed static asset → `public, max-age=31536000, immutable` · HTML → `no-cache` (revalidate) · API/user-specific → `private, no-store`.

```bash
# Invalidate (slow, 5–10 min — prefer versioned URLs)
aws cloudfront create-invalidation --distribution-id E123 --paths '/static/*'
```

Cost feel: CloudFront ~$0.085/GB (US/EU, cheaper at volume) + ~$0.012/10k HTTPS requests — usually cheaper than direct origin egress. Cloudflare R2 has no egress fees.
