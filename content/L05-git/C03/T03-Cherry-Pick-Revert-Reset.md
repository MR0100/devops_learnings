...
- Use `git revert` (not `reset`) to undo anything already pushed — it creates a new inverse commit and never rewrites shared history.
- Reserve `git reset --hard` for local, unpushed work, and `git stash` (or a backup branch) before it so a mistake is recoverable.
- When cherry-picking a fix across branches, prefer `-x` to record the source SHA in the message, making later auditing and de-duplication easy.
- Cherry-pick ranges with `A^..B` (note the `^`) to include commit A; remember the range excludes the left endpoint by default.
- After reverting a merge commit, document that re-merging the branch later requires reverting the revert, so the changes aren't silently skipped.
- Default to `--soft`/`--mixed` resets to reshape commits while keeping your changes; reach for `--hard` only when you truly want the working tree wiped.

## Quick Refs

| Goal | Command | Safe on shared history? |
|---|---|---|
| Copy a commit to here | `git cherry-pick <sha>` | yes (new commit) |
| Undo a commit's effect, keep history | `git revert <sha>` | yes (new commit) |
| Undo a merge commit | `git revert -m 1 <merge-sha>` | yes |
| Move branch pointer, keep changes staged | `git reset --soft <ref>` | NO (rewrites) |
| ...keep changes unstaged (default) | `git reset --mixed <ref>` | NO |
| ...discard changes entirely | `git reset --hard <ref>` | NO (and destructive) |

Rule of thumb: **revert** to undo published commits, **reset** only on local/unpushed history, **cherry-pick** to port a fix across branches.
