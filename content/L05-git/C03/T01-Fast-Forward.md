...
- Expecting a merge commit when the branches fast-forward — by default Git moves the pointer with no commit, so the topic branch's existence vanishes from history; use `--no-ff` if you want it preserved.
- Using `--squash` and then thinking the branch is "merged" — squash does NOT create a merge link, so `git branch --merged` won't list it and re-merging can duplicate commits.
- Reverting a merge commit and later trying to re-merge the same branch, only to find Git thinks the changes are already present — reverting a merge needs care (`git revert -m 1`) and a follow-up un-revert to re-merge.
- Assuming `--ff-only` is safe to script into pulls without expecting failures; it correctly *refuses* when a real merge is needed, which surprises people who treat the error as a bug.
- Resolving conflicts only on the surviving side during a three-way merge and forgetting the merge base context, producing a "resolution" that drops one side's intended change.
- Treating an octopus merge as a way to merge conflicting branches — octopus refuses if any branch conflicts; it's only for clean, independent integrations.

## Best Practices

- Choose a merge policy deliberately: `--ff-only` for a strictly linear trunk, `--no-ff` when you want every feature to leave a visible merge bubble for auditing/revert.
- Keep feature branches short and rebase onto the target before merging so the merge is trivial (often a fast-forward) and conflicts surface early.
- Always finish a merge with a review of `git diff` against both parents, not just a `git status` clean check, to catch silently dropped changes.
- Use `git merge --abort` the moment a conflict resolution feels wrong rather than committing a half-baked merge.
- Enable `rerere` (`git config rerere.enabled true`) on long-lived branches so repeated conflict resolutions are remembered and replayed.
...
git merge --ff-only feature       # fail unless a clean fast-forward
git merge --squash feature        # stage combined diff, NO merge link

# Understand the merge
git merge-base main feature       # common ancestor (the "base" of 3-way)
git log --merges --oneline        # list merge commits
git show <merge-sha>              # inspect a merge

# During / after conflicts
git merge --abort                 # bail out cleanly
git diff --name-only --diff-filter=U   # files still conflicted
git config rerere.enabled true    # remember conflict resolutions

# Revert and re-merge a merge commit
git revert -m 1 <merge-sha>       # undo a merge (keep parent 1)
```

| Situation | Result |
|-----------|--------|
| target has no new commits | fast-forward (pointer move) |
| both sides advanced | three-way merge commit |
| want history of the branch | `--no-ff` |
| want one combined commit | `--squash` (then plain commit) |
