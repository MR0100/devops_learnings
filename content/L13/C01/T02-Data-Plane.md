...
# What the container runtime sees (on the node)
crictl ps                               # running containers
crictl pods                             # pod sandboxes
crictl images
crictl logs <container-id>

# Static pods live here (kubelet watches this dir)
ls /var/lib/kubelet/   /etc/kubernetes/manifests/
...
|---|---|---|
| kubelet | Manages Pods on the node, reports status | systemd service / binary |
| container runtime | Pulls images, runs containers (CRI) | containerd / CRI-O |
| kube-proxy | Programs Service VIP → endpoint rules | DaemonSet (or static) |
| CNI plugin | Pod networking (IP, routes) | DaemonSet + binary |
| CSI plugin | Mounts volumes | DaemonSet + controller |
