...
- Use the native sidecar API (alpha 1.28, on-by-default beta 1.29, GA 1.33)
...
## Native Sidecar (alpha 1.28, beta 1.29, GA 1.33)

The native sidecar API landed as alpha in **1.28**, became **on-by-default beta in 1.29**, and went **GA in 1.33**. From 1.29 onward you can rely on it without a feature gate. Mechanism: an init container with `restartPolicy: Always`:
```yaml
spec:
  initContainers:
  - name: log-shipper
...
For new K8s (1.29+, GA in 1.33): use this.
...
- Relying on pre-1.28 sidecar hacks when the native API (beta 1.29, GA 1.33) is available

## Best Practices

- Use the native sidecar API (beta 1.29, GA 1.33)
...
# Native sidecar API (beta 1.29, GA 1.33)
initContainers:
- name: sidecar
  restartPolicy: Always
containers:
