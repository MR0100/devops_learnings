...
- Write table-driven tests with named subtests (`t.Run(tc.name, ...)`) so each case fails independently and reports clearly.
- Mock at small interfaces you define; use `httptest.Server` for HTTP and `fake.NewSimpleClientset()` / envtest for Kubernetes.
- Mark every reusable assertion helper with `t.Helper()` so failures point at the caller's line.
- Use `t.Cleanup` (not bare `defer`) for teardown, and `t.TempDir()` for scratch files — both auto-clean even on parallel/subtests.
- Run `go test -race ./...` in CI; gate coverage with `go test -coverprofile` and a threshold check.
- Add fuzz tests for parsers/encoders and keep golden files under `testdata/` (an ignored directory by Go tooling).
...
go test -run TestParse -v     # single test, verbose
