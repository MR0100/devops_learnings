...
- Use shared informers and read from the Lister (cache), not the API server, on every loop — it slashes API load and latency.
- Handle write conflicts with `retry.RetryOnConflict`: re-Get the latest object, re-apply your mutation, then Update.
- Grant the ServiceAccount least-privilege RBAC scoped to the exact resources/verbs the controller uses.
- Pass a `context.Context` to every call and set timeouts; never leave a long-lived watch without a context or resync period.
- Prefer `controller-runtime`'s client/manager for new operators over hand-wiring raw client-go.
- Use a rate-limited work queue so failed items requeue with backoff instead of hot-looping.
...
```go
// Shared informer + Lister (read from cache, not API server)
...
```yaml
# Least-privilege RBAC for the controller's ServiceAccount
rules:
  - apiGroups: ["apps"]
