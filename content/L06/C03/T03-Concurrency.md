...
> Note: `time.Tick` leaks its underlying ticker — it can never be stopped or
> garbage-collected. It's fine for the lifetime of a program, but in real code
> (especially in functions that return) use `t := time.NewTicker(d)` with
> `defer t.Stop()` and read from `t.C`.
...
- Bound concurrency with a worker pool or a semaphore channel — don't spawn one goroutine per item against an unbounded input.
- Make the goroutine that owns a channel responsible for closing it, and close exactly once; receivers never close.
- Always give goroutines an exit path: propagate `context.Context` and select on `ctx.Done()` so cancellation stops them.
- Use `sync.WaitGroup` to wait for completion; `wg.Add` before launching, `defer wg.Done()` inside, `wg.Wait()` after the loop.
- Prefer channels for handing off ownership of data; use `sync.Mutex` only for protecting shared mutable state.
- Run `go test -race` (and `go build -race` locally) in CI — most concurrency bugs are invisible without it.
...
var wg sync.WaitGroup
for range runtime.NumCPU() {            // Go 1.22+ range-over-int
    wg.Add(1)
    go func() {
        defer wg.Done()
...
case res := <-resultCh:
    return res, nil
case <-ctx.Done():
    return nil, ctx.Err()              // canceled or deadline exceeded
}

// errgroup: run N tasks, cancel all on first error
...
go vet ./...               # catch common concurrency mistakes
```
