...
- Let tooling enforce style: `ruff format` (or Black) for formatting, `ruff check` for linting — never argue style in review.
- Annotate public function signatures with modern typing (`list[str]`, `str | None`, `dict[str, int]`) and run `mypy --strict` in CI.
- Use the `src/` layout so tests import the installed package, not loose files on `sys.path`.
- Prefer EAFP and context managers (`with open(...)`) over manual checks and `try/finally` resource handling.
- Pin a single `pyproject.toml` as the source of truth for deps, ruff, and mypy config; commit it and the lockfile.
- Default to `pathlib`, f-strings, comprehensions, and the stdlib before reaching for a dependency.

## Quick Refs

```toml
# pyproject.toml — modern lint + type config
[tool.ruff]
line-length = 100
[tool.ruff.lint]
...
ruff check . --fix         # lint + autofix (incl. import sorting)
mypy src/                  # static type check
uvx ruff check .           # run ruff without installing it
```

```python
# Idiomatic typing (3.10+)
def parse(raw: str | None = None) -> dict[str, int]:
    items: list[str] = (raw or "").split(",")
    return {k: i for i, k in enumerate(items) if k}
```
