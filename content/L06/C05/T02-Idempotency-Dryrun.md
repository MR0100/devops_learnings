...
- Design every mutating operation as "read current state, compute the diff, apply only the delta" so re-running is a safe no-op.
- Use `create-or-update` / upsert semantics (kubectl apply, `INSERT ... ON CONFLICT`) instead of blind create that fails or duplicates on retry.
- Make `--dry-run` show the concrete diff (from -> to), not just "would run"; gate it so no side effects can leak.
- Confirm destructive actions by default; require typing the resource name for high-blast-radius ops, and allow `--yes/--auto-approve` for CI.
- Pass an idempotency key (request ID) to downstream APIs so retries de-duplicate server-side.
- Minimize the plan/apply gap: re-validate or lock state at apply time, or use a saved plan file so you apply exactly what was reviewed.

## Quick Refs

```python
# Idempotent reconcile: diff desired vs actual, apply delta only
def reconcile(name: str, desired: int, *, dry_run: bool, assume_yes: bool) -> None:
    current = get_replicas(name)          # read actual state
    if current == desired:
        log.info("no change: %s already at %d", name, desired)
        return                            # safe no-op on re-run
    print(f"Would scale {name}: {current} -> {desired}")
    if dry_run:
        return
    if not assume_yes and not confirm(f"Scale {name} to {desired}?"):
        raise SystemExit("aborted")
    set_replicas(name, desired)
```

```python
# Idempotency key so downstream retries de-duplicate
headers = {"Idempotency-Key": f"deploy-{service}-{version}"}
client.post(url, json=payload, headers=headers, timeout=10)
```

```bash
# Tools that are idempotent / plan-first by design
terraform plan -out=tf.plan && terraform apply tf.plan   # apply the reviewed plan
kubectl apply -f deploy.yaml          # create-or-update
kubectl diff -f deploy.yaml           # dry-run diff before apply
helm upgrade --install app ./chart    # install or upgrade in one verb
mytool destroy prod --yes             # explicit auto-approve for automation
```
