...
- Emit structured JSON logs (structlog / `slog`) with stable keys so they're queryable in your log backend, not free-text prose.
- Attach a request/correlation ID to every log line and propagate it across services so you can trace one operation end to end.
- Send logs to stderr and keep stdout for machine-readable output; never interleave diagnostics with the tool's data.
- Log at the right level (DEBUG off by default, INFO for milestones, ERROR with context) and make verbosity a flag/env var.
- Never log secrets, tokens, or PII; redact known-sensitive keys before serializing.
- Adopt OpenTelemetry for traces and metrics so logs/traces/metrics share trace IDs and you can correlate across the three pillars.

## Quick Refs

```python
# Structured JSON logging with structlog + request id
import structlog
...
log = structlog.get_logger()
log = log.bind(request_id=req_id, service="deployer")   # context on every line
log.info("deploy.start", app="web", version="v1.2.3")
log.error("deploy.failed", app="web", error=str(err))
```

```go
// Go 1.21+ stdlib structured logging
import "log/slog"
logger := slog.New(slog.NewJSONHandler(os.Stderr, nil)).
    With("service", "deployer", "request_id", reqID)
logger.Info("deploy.start", "app", "web", "version", "v1.2.3")
logger.Error("deploy.failed", "app", "web", "err", err)
```

```python
# OpenTelemetry span + Prometheus-style counter
from opentelemetry import trace, metrics
tracer = trace.get_tracer("deployer")
deploys = metrics.get_meter("deployer").create_counter("deploys_total")

with tracer.start_as_current_span("deploy") as span:
    span.set_attribute("app", "web")
    deploys.add(1, {"app": "web", "result": "success"})
```
