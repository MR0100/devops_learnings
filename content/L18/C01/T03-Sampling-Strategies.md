...
**Junior**: "Why would you sample logs?" — High-volume services generate logs faster than you can afford to store, transport, and search, so sampling keeps a representative subset to cut cost while preserving enough signal to debug.

**Mid**: "How does head sampling differ from tail sampling?" — Head sampling decides at request start (cheap and consistent, but you don't yet know if the request is interesting), while tail sampling buffers the whole request and decides after the outcome is known so you can always keep errors and slow requests.

**Senior**: "When is tail sampling worth its complexity?" — When you must guarantee that errors and outliers are retained while dropping the boring 90%; the trade-off is the memory and buffering needed (e.g. OTel Collector `tail_sampling` holding traces for a decision window) versus head sampling's stateless simplicity.

**Staff**: "Design a sampling strategy for a multi-service platform." — Keep errors and critical endpoints at 100%, head-sample the bulk at a low rate with sticky/consistent hashing so a request is kept or dropped as a whole, layer tail sampling for slow/error retention, set per-service volume caps, and emit the sample rate so analytics can re-weight counts.
