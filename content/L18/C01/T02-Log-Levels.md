...
**Junior**: "Name the standard log levels and what they mean." — TRACE/DEBUG for diagnostics (off in prod), INFO for normal operation, WARN for recoverable issues worth investigating, and ERROR/FATAL for failures, with FATAL meaning the process is terminating.

**Mid**: "When should something be logged at ERROR?" — Only when it's unexpected, user-impacting, and requires human action; a client mistake (4xx) or a slow query is not a server ERROR, since flooding ERROR with expected events destroys its value as an alert signal.

**Senior**: "How do you maintain log-level discipline across a team?" — Document level criteria, ban log-and-rethrow double logging, override noisy libraries per-module to WARN, keep INFO as the prod default with dynamic DEBUG for incidents, and treat a constantly-firing `{level="error"}` query as a bug to fix.

**Staff**: "How do log levels fit into an alerting and cost strategy?" — Tie alerts to ERROR rate so the level is meaningful, sample DEBUG/TRACE and noisy endpoints to cap spend, and prefer structured event types over INFO-everything so signal survives at high RPS without paying to index noise.
