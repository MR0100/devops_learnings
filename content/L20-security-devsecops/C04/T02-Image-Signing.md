...
**Junior**: "Why sign container images?" — A signature lets consumers verify an image came from a trusted build and wasn't swapped or tampered with in the registry, establishing authenticity and integrity.

**Mid**: "What is keyless signing with Sigstore/Cosign?" — Instead of managing long-lived private keys, Cosign uses a short-lived certificate tied to an OIDC identity (e.g. a GitHub Actions workflow) via Fulcio and logs the signature in the Rekor transparency log, so trust is based on identity, not a key you must guard.

**Senior**: "How do you enforce that only signed images run?" — Verify signatures at admission with a policy controller (Kyverno/Cosign policy) that checks the certificate identity and OIDC issuer, rejecting any image not signed by the expected workflow identity.

**Staff**: "How does signing fit into end-to-end supply-chain trust?" — Signing proves who built the artifact, provenance (SLSA) proves how, and SBOM proves what's inside; you bind them by attesting all three to the same image digest and enforcing verification at admission so an unsigned or unprovenanced image can't reach production.
